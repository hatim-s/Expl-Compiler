...
int functionGenerator (FILE *outfile, TreeNode* node) {
    ...
    if (node->varvalue == FUNC_CALL || node->varvalue == METHOD_CALL) {
        ...
        else fprintf (outfile, "CALL FUNC%d\n", getMethodFromClass(node->varname, node->classtype)->funcLabel);
...
int codeGenerator (FILE *outfile, TreeNode* node) {
    ...
    if (node->nodetype == IDENTIFIER_NODE 
        || node->nodetype == ARRAY_NODE || node->nodetype == SELF_NODE) 
    {
    ...
    if (node->nodetype == OPERATOR_NODE) {
        if (strcmp(node->varname, "=") == 0) {
            ...
            if (node->left->nodetype == CLASS_NODE && node->left->varvalue == NEW_NODE) {
                fprintf (outfile, "ADD R%d, 1\n", lt);
                fprintf (outfile, "MOV [R%d], %d\n", lt, node->left->classtype->classIndex);
            }
